    ...
    <Card className="overflow-hidden">
      <div className="border-b border-gray-200 bg-gray-50 px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-row">
          <Button variant="secondary" onClick={handleCopy} className="text-sm py-2 w-full sm:w-auto">
            {copied ? (
          ...
          <Button onClick={handleOpen} className="text-sm py-2 w-full sm:w-auto">
      ...
      <div className="p-0">
        <dl className="divide-y divide-gray-200">
          <div className="px-4 sm:px-6 py-4 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
            <dt className="text-sm font-medium text-gray-500">Short URL</dt>
            <dd className="text-sm text-gray-900 sm:col-span-2 font-medium min-w-0 break-all">
          ...
          <div className="px-4 sm:px-6 py-4 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
            ...
            <dd className="text-sm text-gray-900 sm:col-span-2 min-w-0">
          ...
          <div className="px-4 sm:px-6 py-4 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
            ...
            <dd className="text-sm text-gray-900 sm:col-span-2 break-words">{formatDate(stats.created_at)}</dd>
          </div>

          <div className="px-4 sm:px-6 py-4 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
            <dt className="text-sm font-medium text-gray-500">Expiration</dt>
            <dd className="text-sm text-gray-900 sm:col-span-2 break-words">{formatDate(stats.expires_at)}</dd>
          </div>

          <div className="px-4 sm:px-6 py-4 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
            <dt className="text-sm font-medium text-gray-500">Status</dt>
            <dd className="text-sm text-gray-900 sm:col-span-2">{stats.is_active ? 'Active' : 'Inactive'}</dd>
          </div>
        </dl>
      </div>
    </Card>
